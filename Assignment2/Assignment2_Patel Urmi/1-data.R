...
## [your code here]
arrange(filter(select(df, X, Grad.Rate), Grad.Rate >= 80 & Grad.Rate <= 98), desc(Grad.Rate))
...
DF <- group_by(df, Private)
DF <- summarise(DF, Max = max(Outstate), Min = min(Outstate))
DF

#For Dataset college:

college <- group_by(college, Private)
college <- summarise(college, Max = max(Outstate), Min = min(Outstate))
college
